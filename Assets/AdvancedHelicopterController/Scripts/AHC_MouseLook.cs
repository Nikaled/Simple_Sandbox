...
namespace alelavoie
{
    ...
    public class AHC_MouseLook : MonoBehaviour
    {
        ...
        void FollowTarget()
        {
            Vector3 heliTranslation = TargetObject.transform.position - _lastPosition;
            gameObject.transform.Translate(heliTranslation, Space.World);

            float heliRotation = TargetObject.transform.localEulerAngles.y - _lastRotation.y;

            //Only sync rotation around the Y axis. 
            gameObject.transform.RotateAround(TargetObject.transform.position, Vector3.up, heliRotation);
        ...
        void ReturnToDefaultPosition()
        {
            ...
            float numberOfFramesLeft = Mathf.Ceil(angleToTarget / Mathf.Lerp(0.3f, 5f, FallBackSensitivity));
            ...
            {
                transform.RotateAround(TargetObject.transform.position, -horizontalRotateAroundAxis, horizontalDeltaAngle / numberOfFramesLeft);
            }
            ...
            {
                transform.RotateAround(TargetObject.transform.position, verticalRotateAroundAxis, verticalDeltaAngle / numberOfFramesLeft);
            }
        }


        private Vector3 GetDefaultCameraDirection()
        {
